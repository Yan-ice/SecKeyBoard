...
from NFCutil import RECV_AES_PWD, RECV_CLIENT_CERT, RECV_CLIENT_DH, RECV_CLIENT_DH_SIGNATURE, SEND_CERT, SEND_DH, SEND_DH_SIGNATURE, recv_item, send_apdu
...
def first_step(session_id):
    with Timer("一阶段NFC耗时"):
        ...
        # 将EDCH公钥发送给安卓APP。
        print(f"sending ECDH pub key(also challenge): {toHexString(list(server_pub_bytes))}")
...
def second_step(session_id):
    with Timer("二阶段NFC耗时"):
        ...
        cert = parse_cert_and_print_attestation(client_edch_cert)
        print(f"客户端证书解析成功")
        ...
        client_auth_pub_key.verify(
            client_edch_signature,
            client_edch_public_bytes,
            padding.PKCS1v15(),
            hashes.SHA256()
        )
        # TODO： check verify
